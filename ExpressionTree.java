...
public class ExpressionTree{
  ...
  public String toStringPrefix(){
    ...
    if (isValue()){
      ...
      // otherwise, tree is an operator
      // return operator, left subtree, and right subtree with spaces in between
  ...
  public double evaluate(){
    // if tree is a value, return the value
    if (isValue()){
      return getValue();
    }
    else{
  ...
  private double apply(char op, double a, double b){
    // check and use operator on a and b, return result
    if (op == '+'){
      return a + b;
    }
    if (op == '-'){
    ...
    return 0.0; // so program compiles
  }
  private char op;
